...
const Variable = require('eslint-scope').Variable
...
function variableType(variable) {
    ...
    switch (def.type) {
        case Variable.CatchClause:
            return "'catch' parameter";
        case Variable.Parameter:
            return "parameter";
        case Variable.FunctionName:
            return "function";
        case Variable.ClassName:
            return "class";
        case Variable.Variable: {
            let kindStr = "";
            if (def.parent && def.parent.kind !== "var") {
                kindStr = "'" + def.parent.kind + "' ";
            }
            return "variable";
        }
        case Variable.ImportBinding:
            return "imported object";
        case Variable.TDZ:
            return "TDZ";
        case Variable.ImplicitGlobalVariable:
            return "global"
        default:
    ...
    create(context) {
        ...
        function findVariablesInScope(scope) {
            scope.references.forEach(reference => {
                ...
                // Reports.
                let type = variableType(variable);
                type = type[0].toUpperCase() + type.slice(1)
                context.report({
                    node: reference.identifier,
                    message: type + " '{{name}}' was used before it was defined.",
